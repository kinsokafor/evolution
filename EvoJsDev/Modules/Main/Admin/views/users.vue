    ...
    return all
      .filter((j) => {
        for (var filter in filters) {
          if (filter == "fullname") {
            const pos = getFullname(j)
              .toLowerCase()
              .search(filters[filter].replaceAll("%", "").toLowerCase());
      ...
      .map((i) => {
        ...
        if (i?.profile_picture == undefined) {
          ...
          profile =
            i?.profile_picture.charAt(0) == "/"
              ? i?.profile_picture
              : String(i?.profile_picture).search("http") == -1 && String(i?.profile_picture).substring(0,4) !== "data"
              ? "/" + i?.profile_picture
              : i?.profile_picture;
        }
...
const getLink = (id, role) => {
  if (role in auth.value.roles) {
    if ("profile" in auth.value.roles[role]) {
      return ("/" + auth.value.roles[role].profile + "/" + id)
        .replace("//", "/")
        .replace("//", "/");
    }
  }
